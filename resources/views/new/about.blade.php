...
@section('content')
    <div class="page_about container-alt max-w-4xl">
        ...
        <div class="mb-6">
            @if ($debug == true || $env !== 'production')
                <div role="alert"
                    ...
                    <p class="mb-2 flex items-center gap-x-2 text-orange-600">
                        ...
                        <span class="text-xs/4 font-medium">{{ __('Warning') }}</span>
                    </p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-[2fr_1fr_1fr] gap-4">
                <div class="card card-master shadow-xs p-4">
                    <p class="text-uh-logo dark:text-uh-logo-dark text-sm font-medium leading-4">{{ __('UrlHub') }}</p>
        ...
        <div class="config content-container card card-master">
            ...
            <dl>
                <dt class="mt-2">
                    <code>{{ __('redirection_status_code') }}</code>
                    <p class="font-light text-sm dark:text-dark-400">
                        {{ __('The HTTP status code to use when redirecting a visitor to the original URL.') }}
                    </p>
                </dt>
                <dd class="mt-2">
                ...
                <dt class="mt-2 mb-2">
                    <code>{{ __('Disallowed Keywords') }}</code>
                    <p class="font-light text-sm dark:text-dark-400">
                        {{ __('Strings blocked from use in short URL endings due to system reservations, blacklisting, or potential conflicts.') }}
                    </p>
                </dt>
                <dd class="mt-2 mb-2">
                    <div class="card bg-gray-50! dark:bg-dark-950/50! rounded! px-3 py-2 text-sm">
                        <code class="text-slate-500 dark:text-dark-600">// {{ $disallowedKeys->count() }} Strings</code>
                        <br>
                        ...
                        @if ($keywordInUse->isNotEmpty())
                            <br><br>
                            <code class="text-red-400 dark:text-orange-600">// Unfortunately the list below is already used
                            </code>
                            <br>
                    ...
                    <div class="card bg-gray-50! dark:bg-dark-950/50! rounded! px-3 py-2 text-sm">
                        @if ($domainBlacklist->isNotEmpty())
                            ...
                            @if ($domainInUse->isNotEmpty())
                                <br><br>
                                <code class="text-red-400 dark:text-orange-600">// Unfortunately the list below is already
                                    used</code>
                                <br>
